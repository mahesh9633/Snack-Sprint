...
// NOTE: `mainCategoryId` is the top-level category this product was
// fetched under (passed in by the caller). We prefer it over
// `p.categoryId` — the product's own field — because `p.categoryId` can
// be a leaf/subcategory id, and using it here caused the "Replace Cart
// Items?" popup to fire between products that share the same main
// category but sit in different subcategories.
Product _toProduct(CategoryDataProduct p, {String mainCategoryId = ''}) {
  final raw    = p.defaultImage;
  final imgUrl = (raw.isNotEmpty && raw != 'no_image.png')
      ? '$_tImgBase$raw' : '';
  final int productQty = int.tryParse(p.quantity) ?? 0;
  ...
  return Product(
    ...
    image:              raw,
    imageUrl:           imgUrl,
    category:           mainCategoryId.isNotEmpty ? mainCategoryId : p.categoryId,
    weight:             p.piece.isNotEmpty ? p.piece : '',
    discountPercentage: p.discountPercent.toDouble(),
    quantity:           productQty,
    posQuantity:        productQty,
...
class TrendingScreenState extends State<TrendingScreen>
  ...
  // Product list, walking products + subcategories + nested subcategories.
  // No filtering — every piece present in the response is kept as-is. ──
  List<Product> _parseCategoryDataResult(Map<String, dynamic> result, {String mainCategoryId = ''}) {
    final rawSubs  = result['subcategories'] as List? ?? [];
    final rawProds = result['products']      as List? ?? [];
    final List<Product> out = [];
    ...
    void addProduct(Map<String, dynamic> p) {
      try {
        out.add(_toProduct(CategoryDataProduct.fromJson(p), mainCategoryId: mainCategoryId));
      } catch (_) {}
    }
  ...
  Future<List<Product>> _fetchAllProductsComplete() async {
    ...
    // so later entries simply overwrite earlier ones for the same id
    // (harmless — same product, same complete data, regardless of which
    // category call surfaced it, e.g. shared subcategories). Each
    // response is tagged with the main category id it was fetched
    // under, so every product ends up with the correct main category.
    final Map<String, Product> merged = {};
    for (var i = 0; i < responses.length; i++) {
      final result = responses[i];
      final catId  = categoryIds[i];
      if (result['success'] == true) {
        for (final p in _parseCategoryDataResult(result, mainCategoryId: catId)) {
          merged[p.id] = p;
        }
      }
    }
