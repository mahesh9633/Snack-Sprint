...
class ProfileScreenState extends State<ProfileScreen> {
  ...
  String  _contact          = '';
  int     _rewardPoints     = 0;
  double  _walletBalance    = 0; // ── ADDED: wallet balance shown in profile ──
  ...
  Future<void> _loadUserData() async {
    ...
    try {
      ...
      if (result['success'] == true) {
        ...
        final imgUrl    = data['profile_image'] as String? ?? '';
        final contact   = data['contact']       as String? ?? '';
        // ── ADDED: parse wallet balance from the profile API response ──
              ...
              child: ListView(
                ...
                children: [
                  ...
                  _settingsCard([
                    _settingsRow(Icons.favorite_border, 'Your Wishlist', onTap: _openWishlist),
                    _divider(),
                    // ── ADDED: Wallet Balance row — always visible, shows
                    // ── ₹0 when empty instead of hiding (unlike payment screen) ──
                    _settingsRowWithSub(
                      Icons.account_balance_wallet_outlined,
                      'Wallet Balance',
                      _walletBalance == 0
                          ? '₹0 available'
                          : '₹${_walletBalance.toStringAsFixed(0)} available',
                    ),
                    _divider(),
                    _settingsRowWithSub(
                      Icons.card_giftcard,
