...
import '../config/app_color.dart';
import '../services/pincode_service.dart';
import '../services/add_address_service.dart'; // ✅ NEW — for AddAddressApi.checkDeliveryZone
...
class _AddNewAddressScreenState extends State<AddNewAddressScreen> {
  ...
  // ✅ NEW — tracks whether the current map pin has been confirmed by the
  // backend as inside a store's delivery range. Confirm is blocked until
  // this is true, and re-checked every time the pin moves.
  bool   _zoneVerified   = false;
  bool   _verifyingZone  = false;
  Timer? _zoneDebounce;
  ...
  Future<void> _recenterMap() async {
    try {
      ...
      setState(() => _pin = LatLng(pos.latitude, pos.longitude));
      _mapCtrl.move(_pin, 16);
      _onPinChanged(); // ✅ NEW — re-verify after recentering
    } catch (_) {}
  }

  // ✅ NEW — called any time the pin moves (map tap, GPS detect, recenter,
  // pincode geocode). Debounced so rapid taps don't spam the backend;
  // verifies the LATEST pin position only.
  void _onPinChanged() {
    setState(() => _zoneVerified = false);
    _zoneDebounce?.cancel();
    _zoneDebounce = Timer(const Duration(milliseconds: 500), _verifyPinZone);
  }

  Future<void> _verifyPinZone() async {
    if (!mounted) return;
    setState(() => _verifyingZone = true);
    ...
    if (!mounted) return;
    setState(() {
      _verifyingZone = false;
      _zoneVerified = result.available;
    });
    ...
    if (!result.available) {
      _showSnack(
        result.message.isNotEmpty
            ? result.message
            : 'This location is outside our delivery range. Please pick a different spot.',
        Colors.red,
      );
    }
  }
  ...
  Future<void> _confirmAndSave() async {
    ...
    // ✅ Pin's delivery-zone verification (lat/lng based, not postcode)
    // is mandatory before saving — re-check right now in case it's stale.
    if (!_zoneVerified) {
      setState(() { _saving = true; _saveError = ''; });
      await _verifyPinZone();
      if (!mounted) return;
      setState(() => _saving = false);

      if (!_zoneVerified) {
        setState(() => _saveError = 'Please pin a location within our delivery range before saving.');
        return;
      }
    }
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 8),
          // ✅ NEW — live zone-verification status for the current pin
          if (_verifyingZone)
            Row(children: [
              const SizedBox(
                width: 12, height: 12,
              ...
              const SizedBox(width: 8),
              Text('Checking delivery range…',
                  style: TextStyle(fontSize: 12, color: Colors.grey[600])),
            ])
          else if (_zoneVerified)
            Row(children: [
              const Icon(Icons.check_circle, color: Colors.green, size: 14),
              const SizedBox(width: 6),
              const Text('This location is within our delivery range',
            ...
            Row(children: [
              ...
              const SizedBox(width: 6),
              Expanded(
                child: Text('Location not verified yet — tap the map to pin your exact spot',
              ...
              child: Stack(children: [
                FlutterMap(
                  ...
                  options: MapOptions(
                    ...
                    onTap: (tapPos, latlng) {
                      setState(() => _pin = latlng);
                      _onPinChanged(); // ✅ NEW — re-verify against backend
                    },
                  ),
                  children: [
                    TileLayer(
      ...
      bottomNavigationBar: SafeArea(
        child: Padding(
          ...
          child: SizedBox(
            height: 54,
            child: ElevatedButton(
              onPressed: (_saving || !_zoneVerified) ? null : _confirmAndSave,
              style: ElevatedButton.styleFrom(
                backgroundColor: AppColors.buttonPrimary,
                disabledBackgroundColor:  AppColors.buttonPrimaryDisabled,
                shape: RoundedRectangleBorder(
              ...
              child: _saving
                ...
                child: CircularProgressIndicator(
                  ...
                  : Text(
                _zoneVerified ? 'Confirm & Save Address' : 'Pin a location in delivery range',
                style: const TextStyle(
                    color: Colors.white,
                    fontSize: 16,
                    fontWeight: FontWeight.bold),
