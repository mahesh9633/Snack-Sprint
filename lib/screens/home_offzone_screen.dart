      ...
      if (categoryResult['success'] == true) {
        ...
        for (final s in rawSubs) {
          ...
          for (final p in subProds) {
            ...
            final img     = pMap['image']?.toString() ?? '';
            final url     = _buildImageUrl(img, base);
            // ── FIXED: always use the main/top-level category this
            // subcategory loop belongs to (`catId`), never the product's
            // own (leaf) `category_id`. Using the leaf id here caused the
            // "Replace Cart Items?" popup to fire between products in the
            // same main category but different subcategories. ──
            final pCatId  = catId;
            final qty     = int.tryParse(pMap['pos_quentity']?.toString() ?? '0') ?? 0;
