...
// NOTE: `mainCategoryId` is the top-level category the customer is
// currently browsing (passed down from whichever screen/tap triggered
// this). We always prefer it over `p.categoryId`, because `p.categoryId`
// is the product's own (possibly leaf/subcategory) id and using it here
// was causing the "Replace Cart Items?" popup to fire between two
// products that are really in the same main category.
  ...
  void _goToAllProducts(String title, List<CategoryDataProduct> products) {
    Navigator.push(context, MaterialPageRoute(
      builder: (_) => AllProductsScreen(
        title: title,
        products: products,
        mainCategoryId: widget.parentCat.categoryId,
      ),
    ));
  }
            ...
            SliverPadding(
              ...
              sliver: SliverGrid(
                ...
                delegate: SliverChildBuilderDelegate(
                      (_, i) => RepaintBoundary(
                      child: ProductCard(
                        product: _toProduct(products[i], mainCategoryId: widget.parentCat.categoryId),
                        imageHeight: imgH,
                      )),
                  childCount: products.length > _previewMax
                      ? _previewMax
      ...
      body: widget.products.isEmpty
        ...
        child: GridView.builder(
          ...
          itemBuilder: (_, i) {
            ...
            return RepaintBoundary(
                child: ProductCard(
                  product: _toProduct(visible[i], mainCategoryId: widget.mainCategoryId),
                  imageHeight: imgH,
                ));
          },
        ),
