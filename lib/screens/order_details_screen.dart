...
//
//   // ── Cancellation-request pending state (persisted locally until admin
//   // approves — approval flips the real order status to Cancelled). ───────
...
//       // Admin approved the request and the order is really cancelled now
//       // — the pending banner is no longer needed.
//       if (_isCancelled && _cancelRequestPending) {
...
//           children: [
//             Text('Are you sure you want to cancel order #${widget.orderId}?'),
...
//
//     final token = await SessionManager.getToken() ?? '';
//     print('DEBUG CANCEL -> orderId: "${widget.orderId}", token: "$token"');
//     final result = await CancelOrderApi.requestCancellation(
//       token: token,
...
//               ? result.message
//               : 'Failed to submit cancellation request. Try again.'),
...
//       final rawGrandTotal = double.tryParse(invoice['total_received'].toString())  ?? 0;
//       advanceUsed  = double.tryParse(invoice['advance_used']?.toString() ?? '0') ?? 0;
//       paidByWallet = rawGrandTotal <= 0 && advanceUsed > 0;
//       grandTotal   = paidByWallet ? advanceUsed : rawGrandTotal;
...
//     (cashAmt <= 0 && upiAmt <= 0 && advanceUsed > 0) ? 'Wallet' : paymentMethod;
//
...
//                       if (advanceUsed > 0)
//                         cRow('Wallet Used', 'Rs.${_fmt(advanceUsed)}',
//                             lc: const PdfColor(0.416, 0.106, 0.573),
//                             vc: const PdfColor(0.416, 0.106, 0.573)),
...
//                     pw.SizedBox(height: 4),
//                     box(pw.Column(children: [
//                       cRow('Method', effectivePaymentMethod, lc: grey700, vc: blue),
//                       if (cashAmt > 0) cRow('Cash', 'Rs.${_fmt(cashAmt)}', lc: grey700, vc: black87),
//                       if (upiAmt  > 0) cRow('UPI',  'Rs.${_fmt(upiAmt)}',  lc: grey700, vc: black87),
//                       if (advanceUsed > 0)
//                         cRow('Wallet', 'Rs.${_fmt(advanceUsed)}',
//                             lc: grey700, vc: const PdfColor(0.416, 0.106, 0.573)),
...
//     final rawGrandTotal = double.tryParse(inv['total_received'].toString())  ?? 0;
//     final advanceUsed   = double.tryParse(inv['advance_used']?.toString() ?? '0') ?? 0;
...
//     final advanceUsed =
//         double.tryParse(inv?['advance_used']?.toString() ?? '0') ?? 0;
//     final rawGrandTotal = double.tryParse(inv?['total_received']?.toString() ?? '0') ?? 0;
//     final paidByWallet  = rawGrandTotal <= 0 && advanceUsed > 0;
//     final grandTotal    = paidByWallet ? advanceUsed : rawGrandTotal;
...
//       child: Column(children: [
//         _infoRow(Icons.payments_outlined, 'Method', effectivePaymentMethod),
//         if (cashAmt > 0) ...[
//           const SizedBox(height: 8),
...
//         if (advanceUsed > 0) ...[
//           const SizedBox(height: 8),
//           _infoRow(Icons.account_balance_wallet, 'Wallet', '₹${_fmt(advanceUsed)}',
...
class _OrderDetailScreenState extends State<OrderDetailScreen>
  ...
  // Preferred source: `_data['cancellation_request']` from the server, once
  // your backend endpoint exposes it — this gives real pending/approved/
  // rejected states with the admin's comment.
  // Fallback source: a local "just submitted" flag in SharedPreferences,
  // used ONLY until the server field is live (or for the brief moment
  // right after submit, before the next fetch). The local flag can only
  // ever say "pending" — it has no way to know about rejection.
  Map<String, dynamic>? get _cancellationRequest {
    final raw = _data?['cancellation_request'];
    if (raw is Map<String, dynamic>) return raw;
    return null; // handles PHP's empty-array [] for "no request" case
  }

  int? get _cancellationStatus => _cancellationRequest != null
      ? int.tryParse(_cancellationRequest!['status'].toString())
      : null;

  bool get _cancelApproved => _cancellationStatus == 1;
  bool get _cancelRejected => _cancellationStatus == 2;

  // Effective "pending" = server says pending(0), OR (server has no info
  // yet AND our local flag says we just requested it AND order isn't
  // already cancelled for real).
  bool get _cancelPending {
    if (_cancellationStatus != null) return _cancellationStatus == 0;
    return _cancelRequestPendingLocal && !_isCancelled;
  }

  String get _cancelAdminComment =>
      _cancellationRequest?['admin_comment']?.toString() ?? '';

  // ── Local optimistic flag (fallback only) ────────────────────────────────
  bool _cancelRequestPendingLocal = false;
  String get _cancelPendingPrefsKey => 'cancel_requested_${widget.orderId}';

  Future<void> _loadCancelPendingFlag() async {
    final prefs = await SharedPreferences.getInstance();
    final pending = prefs.getBool(_cancelPendingPrefsKey) ?? false;
    if (mounted) setState(() => _cancelRequestPendingLocal = pending);
  }

  Future<void> _clearCancelPendingFlag() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.remove(_cancelPendingPrefsKey);
    if (mounted) setState(() => _cancelRequestPendingLocal = false);
  }
  @override
  void initState() {
    super.initState();
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (json['status'] == 'success') {
          ...
          // Server now has authoritative info (present field, or order is
          // really cancelled) — the local "just submitted" flag is no
          // longer needed either way.
          if (_cancellationRequest != null || _isCancelled) {
            _clearCancelPendingFlag();
          }
          _fetchTrackOrder(widget.orderId);
          _fetchAddresses();
  ...
  Future<void> _cancelOrder() async {
    ...
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text(_cancelRejected ? 'Request Cancellation Again' : 'Cancel Order'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
    ...
    if (result.success) {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setBool(_cancelPendingPrefsKey, true);
      if (mounted) setState(() => _cancelRequestPendingLocal = true);
  ...
  // ✅ UPDATED — 3-state banner: normal / pending / rejected, driven by
  // `_data['cancellation_request']` from the server instead of local prefs.
  Widget _buildCancelBanner() {
    if (_isCancelled) return const SizedBox.shrink();
      ...
      return Container(
        ...
        child: Column(
          ...
          children: [
            ...
            if (_cancelAdminComment.isNotEmpty) ...[
