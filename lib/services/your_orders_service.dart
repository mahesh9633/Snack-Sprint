...
//     advanceUsed:       json['advance_used']?.toString() ?? '0',
//   );
// }
//
// class OrderHistory {
...
// ✅ NEW — mirrors a row in `order_cancellation_requests`
class CancellationRequest {
  ...
  factory CancellationRequest.fromJson(Map<String, dynamic> json) => CancellationRequest(
    requestId:     json['request_id']?.toString() ?? '',
    status:        int.tryParse(json['status']?.toString() ?? '0') ?? 0,
    reason:        json['reason']?.toString() ?? '',
    adminComment:  json['admin_comment']?.toString() ?? '',
    dateRequested: json['date_requested']?.toString() ?? '',
    dateProcessed: json['date_processed']?.toString(),
  );
}
...
class Order {
  ...
  final List<OrderHistory> history;
  final List<TrackingStep> tracking;
  final CancellationRequest? cancellationRequest; // ✅ NEW
  ...
  factory Order.fromJson(Map<String, dynamic> json) => Order(
    ...
    tracking: (json['tracking'] as List<dynamic>? ?? [])
        .map((t) => TrackingStep.fromJson(t as Map<String, dynamic>))
        .toList(),
      cancellationRequest: json['cancellation_request'] is Map<String, dynamic>
          ? CancellationRequest.fromJson(json['cancellation_request'] as Map<String, dynamic>)
          : null,
  );
}
